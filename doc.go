/*
Package nougat is a Go HTTP client library for creating and sending API requests.
Slings store HTTP Request properties to simplify sending requests and decoding
responses. Check the examples to learn how to compose a Nougat into your API
client.
Usage
Use a Nougat to set path, method, header, query, or body properties and create an
http.Request.
	type Params struct {
	    Count int `url:"count,omitempty"`
	}
	params := &Params{Count: 5}
	req, err := nougat.New().Get("https://example.com").QueryStruct(params).Request()
	client.Do(req)
Path
...
path will be resolved relative to the existing URL.
	...
	req, err := nougat.New().Base("https://example.com/").Path("foo/").Path("bar").Request()
Use Get, Post, Put, Patch, Delete, or Head which are exactly the same as Path
except they set the HTTP method too.
	req, err := nougat.New().Post("http://upload.com/gophers")
Headers
Add or Set headers for requests created by a Nougat.
	s := nougat.New().Base(baseUrl).Set("User-Agent", "Gophergram API Client")
...
Use QueryStruct to encode a struct as query parameters on requests.
	...
	githubBase := nougat.New().Base("https://api.github.com/").Client(httpClient)
...
header.
Plain Body
Use Body to set a plain io.Reader on requests created by a Nougat.
	body := strings.NewReader("raw body")
	req, err := nougat.New().Base("https://example.com").Body(body).Request()
Set a content type header, if desired (e.g. Set("Content-Type", "text/plain")).
Extend a Nougat
Each Nougat generates an http.Request (say with some path and query params)
each time Request() is called, based on its state. When creating
different slings, you may wish to extend an existing Nougat to minimize
duplication (e.g. a common client).
Each Nougat instance provides a New() method which creates an independent copy,
so setting properties on the child won't mutate the parent Nougat.
	const twitterApi = "https://api.twitter.com/1.1/"
	base := nougat.New().Base(twitterApi).Client(authClient)
	// statuses/show.json Nougat
	tweetShowSling := base.New().Get("statuses/show.json").QueryStruct(params)
	req, err := tweetShowSling.Request()
	// statuses/update.json Nougat
...
reference the base Nougat and POST to
"https://api.twitter.com/1.1/statuses/show.json/statuses/update.json", which
is undesired.
Recap: If you wish to extend a Nougat, create a new child copy with New().
Receive
